...
「控制與狀態暫存器（Control and Status Register，簡寫為 CSR）」是用來儲存 CPU 設定與狀態的重要暫存器。完整的 CSR 清單可參見官方的 [RISC-V Privileged Specification](https://riscv.org/specifications/privileged-isa/)。
...
```c
__asm__ __volatile__("csrw sscratch, %0" : /* 輸出操作元為空 */ : "r"(123));
```
